SET SEARCH_PATH TO loonWatch,public; --this makes each create table use SEARCH_PATH as the default schema

--drop table if exists loon_locations;
create table if not exists loon_locations (
	locationName text,
	locationTown text,
	locationRegion text,
...
--psql:
--\COPY loon_locations FROM 'C:\Users\jtloo\Documents\VCE\LoonWeb\loonWatchData\csv_import\LoonWatch_Sampling_Locations.csv' DELIMITER ',' CSV HEADER
--sql: (in Windows Explorer alter csv_import folder >properties >security: add group 'everyone'	with full privs)
--COPY loon_locations FROM 'C:\Users\jtloo\Documents\VCE\LoonWeb\loonWatchData\csv_import\LoonWatch_Sampling_Locations.csv' DELIMITER ',' CSV HEADER
select * from loon_locations;

--constrain locationName to be unique
ALTER TABLE loon_locations ADD CONSTRAINT loon_location_unique UNIQUE(locationName);
--add foreign key waterBodyId 
ALTER TABLE loon_locations ADD COLUMN waterBodyId text;
ALTER TABLE loon_locations ADD CONSTRAINT fk_waterbody_id FOREIGN KEY (waterBodyId) REFERENCES vt_water_body (wbTextId);
--add foreign key locationTownId
ALTER TABLE loon_locations ADD COLUMN locationTownId integer;
ALTER TABLE loon_locations ADD CONSTRAINT fk_loon_location_town_id FOREIGN KEY (locationTownId) REFERENCES vt_town ("townId");

--fix incorrect town names in loon_locations
update loon_locations set locationTown='Hubbardton' where locationTown='Hubbarton';

--set loon location townId from vt_town
update loon_locations l 
...
select locationTown, locationName from loon_locations l
...
update loon_locations set locationName=TRIM(BOTH FROM locationName);
--update Silver Lake in Barnard to have capitalized parenthetial townName
update loon_locations set locationName='Silver (Barnard)' where locationName='Silver (barnard)';

-- fix loon location towns that are misspelled or incorrect
update loon_locations set locationTown='Townshend' where locationTown='Townsend';
update loon_locations set locationTown='Lunenburg' where locationTown='Lunenberg';
update loon_locations set locationTown='Hartford' where locationTown='Quechee';

--alter town for lake 'Memphremagog' to 'Derby'
update loon_locations set locationTown='Derby' where locationName = 'Memphremagog';
--alter town for lake 'South Bay' from 'Newport' to 'Newport City'
update loon_locations set locationTown='Newport City' where locationName='South Bay';
--fix other loon locations whose towns are incorrect
update loon_locations set locationTown='Elmore' where locationName='Hardwood';
update loon_locations set locationTown='Eden' where locationName='South (Eden)';
...
select * from loon_locations where lower(locationName) like '%adamant%';
...
DROP TABLE tmp;
UPDATE loon_locations set waterBodyId='ADAMANT' WHERE locationName='Adamant';	
...
select * from loon_locations where lower(locationName) like '%nelson%';
select * from vt_water_body where wbTextId like '%FOREST (CALAIS)%';
update loon_locations set waterBodyId='FOREST (CALAIS)' WHERE locationName='Nelson';
...
select * from loon_locations where lower(locationName) like '%little salem%';
select * from vt_water_body where wbTextId like '%SALEM%';
update loon_locations set waterBodyId='SALEM' WHERE locationName='Little Salem';
...
UPDATE tmp SET wbArea = (SELECT locationArea FROM loon_locations WHERE locationName='Little Salem');
INSERT INTO vt_water_body SELECT * from tmp;
...
select * from loon_locations where lower(locationName) like '%memphremagog%';
select * from vt_water_body where wbTextId like '%MEMPHREMAGOG%';
update loon_locations set waterBodyId='MEMPHREMAGOG' WHERE locationName='Memphremagog';

--Lake Lamoille didn't match because loon_locations had it in Hyde Park but vt_water_bodies has it in Morristown
update loon_locations set locationTown='Morristown' WHERE locationName='Lamoille';
update loon_locations set locationTownId=(select "townId" from vt_town where "townName"='Morristown') 
	where locationName='Lamoille';

--Townsend Res. is misspelled
--vt_water_bodies has a Townshend Reservoir
select * from loon_locations where lower(locationName) like '%townsend%';
update loon_locations set locationName='Townshend Res.' where locationName='Townsend Res.';
select * from vt_water_body where wbTextId like '%TOWNSHEND%';
update loon_locations set waterBodyId='TOWNSHEND' WHERE locationName='Townshend Res.';

--Happenstance Farm is tiny, private, and isn't in vt_water_body
select * from loon_locations where lower(locationName) like '%happenstance%';
select * from vt_water_body where lower(wbTextId) like '%happenstance%'; --Does not exist
update loon_locations set waterBodyId='HAPPENSTANCE' WHERE locationName='Happenstance Farm';
...
update loon_locations set locationName='Wrightsville' where locationName='WrightSurveyille';
update loon_locations set locationTown='East Montpelier' where locationName='Wrightsville';
update loon_locations set waterBodyId='WRIGHTSVILLE' WHERE locationName='Wrightsville';
...
*/
ALTER TABLE loon_locations ADD COLUMN exportName text;
UPDATE loon_locations set exportName=COALESCE(waterBodyId, TRANSLATE(UPPER(locationName),'''.-:,',''));
UPDATE loon_locations SET exportName=REGEXP_REPLACE(exportName,' +',' ','g');
